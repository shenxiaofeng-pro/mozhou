...
- [ ] 在 1040、900、760 像素宽度提供可访问的 AI 导演入口。
- [ ] 增加项目概要与章节正文按需读取接口，旧 Workspace 契约并存一版。
- [ ] 建立 v1→v2→v3 显式顺序迁移与失败恢复验证。
- [ ] 为桌面 sidecar 和 Web API 请求增加启动期随机会话令牌。
- [ ] 执行 M1 完整自动化、真实浏览器和桌面验收。
