...
def test_database_upgrade_creates_one_backup_and_records_schema_version(tmp_path: Path) -> None:
    ...
    backups = list((tmp_path / "backups").glob("mozhou-before-v6-*.db"))
    ...
    assert list((tmp_path / "backups").glob("mozhou-before-v6-*.db")) == backups
...
def test_database_runs_v1_and_v2_fixtures_to_v6_without_losing_data(
    ...
    assert len(list((tmp_path / "backups").glob("mozhou-before-v6-*.db"))) == 1
