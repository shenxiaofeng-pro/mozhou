...
> 总体状态：M0、M1 已完成并验收；M2 的任务核心、拆书、写章和任务中心已完成，尚待桌面恢复验收
...
| M2 可恢复 AI 任务运行时 | 进行中 | 任务核心、可恢复长篇拆书、章纲/写章和全局任务中心已完成；待桌面恢复与 M2 收尾验收 |
| M3 模型网关、密钥与用量治理 | 未开始 | — |
| M4 长篇上下文编译器 | 未开始 | — |
| M5 全局拆书库与现实资料导入 | 未开始 | — |
| M6 原创性门禁与蓝图工作流 | 未开始 | — |
...
- 执行桌面 sidecar 强制中断、过期租约恢复、已完成 Artifact 复用和 UI 继续操作验收；通过完整 `verify` 与远端 PR 门后收尾 M2。
