...
> 总体状态：M0 至 M4 已完成；M5 数据模型与归档边界已落地，尚未达到总体 Definition of Done
...
| M5 全局拆书库与现实资料导入 | 进行中 | 全局参考资产、项目关联、v8 无损迁移与默认/完整归档边界已通过专项验证 |
| M6 原创性门禁与蓝图工作流 | 未开始 | — |
...
- API 全量 155/155 通过；其中覆盖一次导入、两项目关联、解除关联不删资产、v8→v9 原文哈希与 ID 无损迁移、默认/完整归档和 v1 旧归档恢复。
- Web 35/35、lint、TypeScript/Python 严格类型检查全部通过。
